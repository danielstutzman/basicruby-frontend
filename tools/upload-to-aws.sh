...
s3cmd sync . --exclude="*" --rinclude="^(index.html|test.html|sitemap.txt|robots.txt)$" s3://basicruby-frontend-danstutzman --acl-public --add-header 'Content-Encoding:gzip' --add-header="Cache-Control:public,max-age=0"
...
s3cmd sync . --exclude="*" --rinclude="[0-9a-f]{16}.(js|css)" s3://basicruby-frontend-danstutzman --acl-public --add-header="Cache-Control:public,max-age=31536000" --add-header 'Content-Encoding:gzip'

# Sync non-gzipped but version filed (e.g. images) with 1 year expiration
s3cmd sync . --exclude="*" --rinclude="[0-9a-f]{16}" s3://basicruby-frontend-danstutzman --acl-public --add-header="Cache-Control:public,max-age=31536000"
...
for DIR in static*; do
  cd $DIR
  s3cmd sync . --add-header 'Content-Encoding:gzip' --add-header "Cache-Control:public,max-age=0" s3://basicruby-frontend-danstutzman --acl-public --default-mime-type=text/html
  cd ..
done
